...
#define AU 1.5E11
// range coeficient
#define range_AU_coef 10E-14//tmp about
...
class Mercury: public planet    // Mercury 0.4AU
{
    //protected:
    public:
        Mercury(basis o_b, star* sun) : planet(Planet_absolute_mass(Mercury_mass),basis(o_b - vector(0.4 * 1000)),Mercury_color,Planet_scaled_size(Mercury_size),sun)
        {
            (*(location*)this)[0] -= 0.4 * 1000; //tmp
        }
};

class Venus: public planet      // Venus 0.7AU
{
    //protected:
    public:
        Venus(basis o_b, star* sun) : planet(Planet_absolute_mass(Venus_mass),basis(o_b - vector(0.7 * 1000)),Venus_color,Planet_scaled_size(Venus_size),sun)
        {
            //(*(location*)this)[0] -= 0.7 * 1000; //tmp
        }
};

class Earth: public planet      // Earth 1AU
{
    //protected:
    public:
        Earth(basis o_b, star* sun) : planet(Planet_absolute_mass(Earth_mass),basis(o_b - vector(1000)),Earth_color,Planet_scaled_size(Earth_size),sun,1,0.05) //tmp moon
        {
            //(*(location*)this)[0] -= 1000; //tmp
        }
};

class Mars: public planet       // Mars 1.5AU
{
    //protected:
    public:
        Mars(basis o_b, star* sun) : planet(Planet_absolute_mass(Mars_mass),basis(o_b - vector(1.4 * 1000)),Mars_color,Planet_scaled_size(Mars_size),sun)
        {
            //(*(location*)this)[0] -= 1.4 * 1000; //tmp
        }
};

class Jupiter: public planet    // Jupiter 5.2AU
{
    //protected:
    public:
        Jupiter(basis o_b, star* sun) : planet(Planet_absolute_mass(Jupiter_mass),basis(o_b - vector(5.2 * 1000)),Jupiter_color,Planet_scaled_size(Jupiter_size),sun,4,0.5) //tmp satelites
        {
...
class Saturn: public planet     // Saturn 9.5AU
{
    //protected:
    public:
        Saturn(basis o_b, star* sun) : planet(Planet_absolute_mass(Saturn_mass),basis(o_b - vector(9.5 * 1000)),Saturn_color,Planet_scaled_size(Saturn_size),sun)
        {
...
class Uranus: public planet     // Uranus 19.6AU
{
    //protected:
    public:
        Uranus(basis o_b, star* sun) : planet(Planet_absolute_mass(Uranus_mass),basis(o_b - vector(19.6 * 1000)),Uranus_color,Planet_scaled_size(Uranus_size),sun)
        {
            //(*(location*)this)[0] -= 19.6 * 1000; //tmp
        }
};

class Neptune: public planet    // Neptune 30AU
{
    //protected:
    public:
        Neptune(basis o_b, star* sun) : planet(Planet_absolute_mass(Neptune_mass),basis(o_b - vector(30 * 1000)),Neptune_color,Planet_scaled_size(Neptune_size),sun)
        {
            //(*(location*)this)[0] -= 30 * 1000; //tmp
        }
};

class Pluto: public planet      // Pluto 39AU
{
    //protected:
    public:
        Pluto(basis o_b, star* sun) : planet(Planet_absolute_mass(Pluto_mass),basis(o_b - vector(39 * 1000)),Pluto_color,Planet_scaled_size(Pluto_size),sun)
        {
